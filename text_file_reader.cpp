...
TextFileReader::TextFileReader(FILE *fp, std::string filetype)
    : filetype(std::move(filetype)), closefp(false), line(nullptr), fp(fp),
...
void TextFileReader::set_bufsize(int newsize) {
  if (newsize < 100) {
    throw FileReaderException(
        fmt::format("line buffer size {} for {} file too small, must be > 100",
                    newsize, filetype));
  }
  delete[] line;
  bufsize = newsize;
...
void TextFileReader::rewind() { ::rewind(fp); }
...
char *TextFileReader::next_line(int nparams) {
  ...
  if (ignore_comments && (ptr = strchr(line, '#')))
    *ptr = '\0';
  ...
  while (nwords == 0 || nwords < nparams) {
    ...
    if (ptr == nullptr) {
      if (nwords > 0 && nwords < nparams) {
        throw EOFException(
            fmt::format("Incorrect format in {} file! {}/{} parameters",
                        filetype, nwords, nparams));
      }
      return nullptr;
    }
    if (ignore_comments && (ptr = strchr(line, '#')))
...
void TextFileReader::next_dvector(double *list, int n) {
  ...
  while (i < n) {
    ...
    if (ptr == nullptr) {
      if (i < n) {
        throw FileReaderException(fmt::format(
            "Incorrect format in {} file! {}/{} values", filetype, i, n));
      }
    }
    ValueTokenizer values(line);
    while (values.has_next() && i < n) {
      list[i++] = values.next_double();
    }
  }
}
ValueTokenizer TextFileReader::next_values(int nparams,
