...
void Neighbor::init() {
  ...
  if (style == Neighbor::MULTI) {
    ...
    memory->grow(cutcollectionsq, ncollections, ncollections,
                 "neigh:cutcollectionsq");
...
void Neighbor::print_pairwise_info() {
  ...
  if (binsizeflag)
    binsize = binsize_user;
  else if (style == Neighbor::BIN)
    binsize = 0.5 * cutneighmax;
  else
    binsize = 0.5 * cutneighmin;
  if (binsize == 0.0)
    binsize = bbox[0];
  int nperpetual = 0;
  int noccasional = 0;
  ...
  out +=
      fmt::format("  update: every = {} steps, delay = {} steps, check = {}\n",
                  every, delay, dist_check ? "yes" : "no");
  out +=
      fmt::format("  max neighbors/atom: {}, page size: {}\n", oneatom, pgsize);
  ...
  if (style != Neighbor::NSQ)
    out += fmt::format("  binsize = {:.8g}, bins = {:g} {:g} {:g}\n", binsize,
                       ...
                       ceil(bbox[2] / binsize));
  out += fmt::format(
      "  {} neighbor lists, perpetual/occasional/extra = {} {} {}\n", nlist,
      nperpetual, noccasional, nextra);
  for (i = 0; i < nlist; i++) {
    rq = requests[i];
    if (rq->pair) {
      ...
      if (pname)
        ...
        out += fmt::format("  ({}) pair (none)", i + 1);
    } else if (rq->fix) {
      out += fmt::format("  ({}) fix {}", i + 1, ((Fix *)rq->requestor)->style);
    } else if (rq->compute) {
      out += fmt::format("  ({}) compute {}", i + 1,
    ...
    if (rq->granonesided)
      out += ", onesided";
    if (rq->respamiddle)
      out += ", respa outer/middle/inner";
    else if (rq->respainner)
      out += ", respa outer/inner";
    if (rq->bond)
      out += ", bond";
    if (rq->omp)
      out += ", omp";
...
void Neighbor::modify_params(int narg, char **arg) {
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "every") == 0) {
      ...
      if (iarg + 2 > narg)
        utils::missing_cmd_args(FLERR, "neigh_modify cluster", error);
      cluster_check = utils::logical(FLERR, arg[iarg + 1], false, lmp);
      iarg += 2;
    } else if (strcmp(arg[iarg], "include") == 0) {
      if (iarg + 2 > narg)
        utils::missing_cmd_args(FLERR, "neigh_modify include", error);
      includegroup = group->find(arg[iarg + 1]);
      if (includegroup < 0)
        error->all(FLERR, "Invalid include keyword: group {} not found",
                   arg[iarg + 1]);
      ...
      if (iarg + 2 > narg)
        utils::missing_cmd_args(FLERR, "neigh_modify exclude", error);
      if (strcmp(arg[iarg + 1], "type") == 0) {
        if (iarg + 4 > narg)
          utils::missing_cmd_args(FLERR, "neigh_modify exclude type", error);
        ...
        if (iarg + 4 > narg)
          utils::missing_cmd_args(FLERR, "neigh_modify exclude group", error);
        ...
        if (ex1_group[nex_group] == -1)
          error->all(FLERR, "Invalid exclude group keyword: group {} not found",
                     arg[iarg + 2]);
        ...
        if (iarg + 3 > narg)
          utils::missing_cmd_args(FLERR, "neigh_modify exclude molecule",
                                  error);
        ...
        if (ex_mol_group[nex_mol] == -1)
          error->all(FLERR, "Invalid exclude keyword:group {} not found",
                     arg[iarg + 2]);
        ...
        error->all(FLERR, "Unknown neigh_modify exclude keyword: {}",
                   arg[iarg + 1]);
    } else if (strcmp(arg[iarg], "collection/interval") == 0) {
      if (style != Neighbor::MULTI)
        error->all(
            FLERR,
            "Cannot use collection/interval command without multi setting");
      if (iarg + 2 > narg)
        utils::missing_cmd_args(FLERR, "neigh_modify collection/interval",
                                error);
      ncollections = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      if (ncollections < 1)
        error->all(FLERR,
                   "Invalid collection/interval keyword: illegal number of "
                   "custom collections: {}",
                   ncollections);
      if (iarg + 2 + ncollections > narg)
        error->all(FLERR,
                   "Invalid collection/interval keyword: expected {} separate "
                   "lists of types",
                   ncollections);
      int i;
      ...
      for (i = 0; i < ncollections; i++) {
        ...
        if (i != 0)
          if (collection2cut[i - 1] >= collection2cut[i])
            error->all(FLERR, "Nonsequential interval cutoffs in "
      ...
      if (style != Neighbor::MULTI)
        error->all(FLERR,
                   "Cannot use collection/type command without multi setting");
      if (iarg + 2 > narg)
        utils::missing_cmd_args(FLERR, "neigh_modify collection/type", error);
      ncollections = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      if (ncollections < 1)
        error->all(FLERR,
                   "Invalid collection/type keyword: illegal number of custom "
                   "collections: {}",
                   ncollections);
      if (iarg + 2 + ncollections > narg)
        error->all(FLERR,
                   "Invalid collection/type keyword: expected {} separate "
                   "lists of types",
                   ncollections);
      ...
      for (i = 0; i < ncollections; i++) {
        std::vector<std::string> words =
            Tokenizer(arg[iarg + 2 + i], ",").as_vector();
        for (const auto &word : words) {
          ...
          for (k = nlo; k <= nhi; k++) {
            if (type2collection[k] != -1)
              error->all(
                  FLERR,
                  "Type specified more than once in collection/type commnd");
            type2collection[k] = i;
          }
        }
      }
