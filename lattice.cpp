...
Lattice::Lattice(LAMMPS *lmp, int narg, char **arg) : Pointers(lmp) {
  ...
  if (strcmp(arg[0], "none") == 0)
    ...
    style = BCC;
  else if (strcmp(arg[0], "fcc") == 0)
    style = FCC;
  else if (strcmp(arg[0], "hcp") == 0)
    style = HCP;
  else if (strcmp(arg[0], "diamond") == 0)
    style = DIAMOND;
  else if (strcmp(arg[0], "sq") == 0)
    style = SQ;
  else if (strcmp(arg[0], "sq2") == 0)
    style = SQ2;
  else if (strcmp(arg[0], "hex") == 0)
    style = HEX;
...
Lattice::~Lattice() { memory->destroy(basis); }
...
int Lattice::right_handed() {
  ...
  if (xy0 * orientz[0] + xy1 * orientz[1] + xy2 * orientz[2] <= 0)
    return 0;
  return 1;
}
...
void Lattice::setup_transform() {
  ...
  priminv[0][0] =
      ...
      determinant;
  priminv[1][0] =
      (primitive[1][2] * primitive[2][0] - primitive[1][0] * primitive[2][2]) /
      determinant;
  priminv[2][0] =
      (primitive[1][0] * primitive[2][1] - primitive[1][1] * primitive[2][0]) /
      determinant;
  priminv[0][1] =
      (primitive[0][2] * primitive[2][1] - primitive[0][1] * primitive[2][2]) /
      determinant;
  priminv[1][1] =
      (primitive[0][0] * primitive[2][2] - primitive[0][2] * primitive[2][0]) /
      determinant;
  priminv[2][1] =
      (primitive[0][1] * primitive[2][0] - primitive[0][0] * primitive[2][1]) /
      determinant;
  priminv[0][2] =
      (primitive[0][1] * primitive[1][2] - primitive[0][2] * primitive[1][1]) /
      determinant;
  priminv[1][2] =
      (primitive[0][2] * primitive[1][0] - primitive[0][0] * primitive[1][2]) /
      determinant;
  priminv[2][2] =
...
void Lattice::lattice2box(double &x, double &y, double &z) {
  ...
  z1 *= scale;
  double xnew =
      rotaterow[0][0] * x1 + rotaterow[0][1] * y1 + rotaterow[0][2] * z1;
  double ynew =
      rotaterow[1][0] * x1 + rotaterow[1][1] * y1 + rotaterow[1][2] * z1;
  double znew =
      rotaterow[2][0] * x1 + rotaterow[2][1] * y1 + rotaterow[2][2] * z1;
...
void Lattice::bbox(int flag, double x, double y, double z, double &xmin,
