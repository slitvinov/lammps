...
public:
  ...
  template <typename TYPE>
  TYPE ***create_ragged(TYPE ***&array, int n1, int *n2, int **n3,
                        const char *name) {
    bigint size, nbytes;
    int i, j;
    size = 0;
  ...
  template <typename TYPE>
  TYPE ***create2d_offset(TYPE ***&, int, int, int, const char *name) {
    fail(name);
    return nullptr;
  }
  ...
  template <typename TYPE>
  TYPE ***create2d_offset(TYPE ***&, int, int, int, int, const char *name) {
    fail(name);
    return nullptr;
  }
  template <typename TYPE>
