...
TokenizerException::TokenizerException(const std::string &msg,
...
Tokenizer::Tokenizer(std::string str, std::string _separators)
    : text(std::move(str)), separators(std::move(_separators)), start(0),
      ntokens(std::string::npos) {
  if (utils::has_utf8(text))
    text = utils::utf8_subst(text);
  reset();
}
...
Tokenizer::Tokenizer(Tokenizer &&rhs)
    : text(std::move(rhs.text)), separators(std::move(rhs.separators)),
      ntokens(rhs.ntokens) {
  reset();
}
...
void Tokenizer::reset() { start = text.find_first_not_of(separators); }
...
void Tokenizer::skip(int n) {
  for (int i = 0; i < n; ++i) {
    if (!has_next())
      throw TokenizerException("No more tokens", "");
...
bool Tokenizer::has_next() const { return start != std::string::npos; }
...
ValueTokenizer::ValueTokenizer(const std::string &str,
                               const std::string &separators)
    : tokens(str, separators) {}
ValueTokenizer::ValueTokenizer(ValueTokenizer &&rhs)
...
std::string ValueTokenizer::next_string() { return tokens.next(); }
...
void ValueTokenizer::skip(int n) { tokens.skip(n); }
size_t ValueTokenizer::count() { return tokens.count(); }
