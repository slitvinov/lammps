...
void inertia_triangle(double *idiag, double *quat, double, double *inertia) {
  double p[3][3], ptrans[3][3], itemp[3][3], tensor[3][3];
  quat_to_mat(quat, p);
  quat_to_mat_trans(quat, ptrans);
  diag_times3(idiag, ptrans, itemp);
...
void tribbox(double *h, double radius, double *dist) {
  ...
  dist[0] =
      radius *
      sqrt(ly * ly * lz * lz + ly * ly * xz * xz - 2.0 * ly * xy * xz * yz +
           lz * lz * xy * xy + xy * xy * yz * yz) /
      (lx * ly * lz);
