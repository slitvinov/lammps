...
void Integrate::ev_setup() {
  ...
  if (nelist_global)
    elist_global = new Compute *[nelist_global];
  if (nelist_atom)
    elist_atom = new Compute *[nelist_atom];
  if (nvlist_global)
    vlist_global = new Compute *[nvlist_global];
  if (nvlist_atom)
    vlist_atom = new Compute *[nvlist_atom];
  if (ncvlist_atom)
    cvlist_atom = new Compute *[ncvlist_atom];
  nelist_global = nelist_atom = 0;
  nvlist_global = nvlist_atom = ncvlist_atom = 0;
}
void Integrate::ev_set(bigint ntimestep) {
  ...
  for (i = 0; i < nelist_global; i++)
    if (elist_global[i]->matchstep(ntimestep))
      flag = 1;
  if (flag)
  ...
  for (i = 0; i < nelist_atom; i++)
    if (elist_atom[i]->matchstep(ntimestep))
      flag = 1;
  ...
  for (i = 0; i < nvlist_global; i++)
    if (vlist_global[i]->matchstep(ntimestep))
      flag = 1;
  if (flag)
  ...
  for (i = 0; i < nvlist_atom; i++)
    if (vlist_atom[i]->matchstep(ntimestep))
      flag = 1;
  if (flag || (tdflag && nvlist_atom))
    vflag_atom = VIRIAL_ATOM;
  flag = 0;
  int cvflag_atom = 0;
  for (i = 0; i < ncvlist_atom; i++)
    if (cvlist_atom[i]->matchstep(ntimestep))
      flag = 1;
  if (flag || (tdflag && ncvlist_atom))
    cvflag_atom = VIRIAL_CENTROID;
  if (vflag_global)
    update->vflag_global = ntimestep;
  if (vflag_atom || cvflag_atom)
    update->vflag_atom = ntimestep;
