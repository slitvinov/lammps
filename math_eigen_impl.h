...
template <typename Entry> void Dealloc2D(Entry ***paaX);
template <typename T> struct realTypeMap { typedef T type; };
template <typename T> struct realTypeMap<std::complex<T>> { typedef T type; };
template <typename T> using real_t = typename realTypeMap<T>::type;
template <typename T>
T inner_prod(const std::vector<T> &v1, const std::vector<T> &v2);
template <typename T> real_t<T> l1_norm(const std::vector<T> &v);
template <typename T> real_t<T> l2_norm(const std::vector<T> &v);
template <typename T1, typename T2> void scalar_mul(T1 c, std::vector<T2> &v);
...
template <typename T> struct VectorRandomInitializer {
public:
  static void init(std::vector<T> &);
};
template <typename T> struct VectorRandomInitializer<std::complex<T>> {
public:
  static void init(std::vector<std::complex<T>> &);
};
template <typename T> inline constexpr int sig_decimal_digit() {
  return (int)(std::numeric_limits<T>::digits *
               std::log10(std::numeric_limits<T>::radix));
}
template <typename T> inline constexpr T minimum_effective_decimal() {
  return std::pow(10, -sig_decimal_digit<T>());
}
template <typename T> class LambdaLanczos {
public:
  LambdaLanczos();
  LambdaLanczos(
  ...
  LambdaLanczos(
      std::function<void(const std::vector<T> &, std::vector<T> &)> mv_mul,
      int matrix_size)
      : LambdaLanczos(mv_mul, matrix_size, true) {}
  ...
  void
  SetMul(std::function<void(const std::vector<T> &, std::vector<T> &)> mv_mul);
...
private:
  static void schmidt_orth(std::vector<T> &,
  ...
  static int num_of_eigs_smaller_than(real_t<T>, const std::vector<real_t<T>> &,
                                      const std::vector<real_t<T>> &);
...
public:
  ...
  real_t<Scalar> PrincipalEigen(ConstMatrix matrix, Vector evector,
                                bool find_max = false);
...
template <typename T> struct ConjugateProduct {
public:
  static T prod(T a, T b) { return a * b; }
};
template <typename T> struct ConjugateProduct<std::complex<T>> {
public:
...
inline T inner_prod(const std::vector<T> &v1, const std::vector<T> &v2) {
  return std::inner_product(
      std::begin(v1), std::end(v1), std::begin(v2), T(),
      [](T a, T b) -> T { return a + b; }, ConjugateProduct<T>::prod);
}
template <typename T> inline real_t<T> l2_norm(const std::vector<T> &vec) {
...
template <typename T> inline void normalize(std::vector<T> &vec) {
  scalar_mul(1.0 / l2_norm(vec), vec);
}
template <typename T> inline real_t<T> l1_norm(const std::vector<T> &vec) {
...
Jacobi<Scalar, Vector, Matrix, ConstMatrix>::Jacobi(int n) {
  _Jacobi(n, nullptr, nullptr);
}
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
Jacobi<Scalar, Vector, Matrix, ConstMatrix>::Jacobi(int n, Scalar **M,
                                                    int *max_idx_row) {
  _Jacobi(n, M, max_idx_row);
}
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::_Jacobi(int n, Scalar **M,
                                                          int *max_idx_row) {
  Init();
  if (M) {
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
Jacobi<Scalar, Vector, Matrix, ConstMatrix>::~Jacobi() {
  if (!is_preallocated)
    Dealloc();
}
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
int Jacobi<Scalar, Vector, Matrix, ConstMatrix>::Diagonalize(
    ConstMatrix mat, Vector eval, Matrix evec, SortCriteria sort_criteria,
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::CalcRot(
    Scalar const *const *M, int i, int j) {
  t = 1.0;
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::ApplyRot(Scalar **M, int i,
                                                           int j) {
  M[i][i] -= t * M[i][j];
  M[j][j] += t * M[i][j];
  M[i][j] = 0.0;
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::ApplyRotLeft(Matrix E, int i,
                                                               int j) {
  for (int v = 0; v < n; v++) {
    Scalar Eiv = E[i][v];
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
int Jacobi<Scalar, Vector, Matrix, ConstMatrix>::MaxEntryRow(
    Scalar const *const *M, int i) const {
  int j_max = i + 1;
  for (int j = i + 2; j < n; j++)
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::MaxEntry(
    Scalar const *const *M, int &i_max, int &j_max) const {
  i_max = 0;
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::SortRows(
    Vector eval, Matrix evec, int n, SortCriteria sort_criteria) const {
  for (int i = 0; i < n - 1; i++) {
    int i_max = i;
    for (int j = i + 1; j < n; j++) {
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::Dealloc() {
  Dealloc2D(&M);
  delete[] max_idx_row;
  max_idx_row = nullptr;
  Init();
...
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
Jacobi<Scalar, Vector, Matrix, ConstMatrix>::Jacobi(
    const Jacobi<Scalar, Vector, Matrix, ConstMatrix> &source) {
  Init();
  SetSize(source.n);
  std::copy(source.max_idx_row, source.max_idx_row + n, max_idx_row);
  for (int i = 0; i < n; i++)
    std::copy(source.M[i], source.M[i] + n, M[i]);
}
template <typename Scalar, typename Vector, typename Matrix,
          typename ConstMatrix>
void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::swap(
    Jacobi<Scalar, Vector, Matrix, ConstMatrix> &other) {
  std::swap(n, other.n);
...
template <typename T> inline LambdaLanczos<T>::LambdaLanczos() {
  this->matrix_size = 0;
...
template <typename T>
inline int LambdaLanczos<T>::run(real_t<T> &eigvalue,
                                 std::vector<T> &eigvec) const {
  ...
  for (int k = 1; k <= this->max_iteration; k++) {
    ...
    const real_t<T> zero_threshold =
        minimum_effective_decimal<real_t<T>>() * 1e-1;
  ...
  for (int k = m - 2; k >= 1; k--) {
    cv[k] =
        ((ev - alpha[k + 1]) * cv[k + 1] - beta[k + 1] * cv[k + 2]) / beta[k];
    for (int i = 0; i < n; i++) {
      eigvec[i] += cv[k] * u[k][i];
    }
  }
...
template <typename T>
inline void
LambdaLanczos<T>::schmidt_orth(std::vector<T> &uorth,
...
template <typename T>
inline real_t<T> LambdaLanczos<T>::find_minimum_eigenvalue(
...
template <typename T>
inline real_t<T> LambdaLanczos<T>::find_maximum_eigenvalue(
    const std::vector<real_t<T>> &alpha,
    const std::vector<real_t<T>> &beta) const {
  real_t<T> eps = this->eps * this->tridiag_eps_ratio;
  real_t<T> pmid = std::numeric_limits<real_t<T>>::max();
...
template <typename T>
inline real_t<T>
LambdaLanczos<T>::tridiagonal_eigen_limit(const std::vector<real_t<T>> &alpha,
...
template <typename T>
inline int
LambdaLanczos<T>::num_of_eigs_smaller_than(real_t<T> c,
                                           const std::vector<real_t<T>> &alpha,
                                           const std::vector<real_t<T>> &beta) {
  real_t<T> q_i = 1.0;
  int count = 0;
...
template <typename T> inline void LambdaLanczos<T>::ChooseOffset() {
...
template <typename T> inline int LambdaLanczos<T>::SetSize(int matrix_size) {
  this->matrix_size = matrix_size;
  this->max_iteration = matrix_size;
  return matrix_size;
}
template <typename T>
inline void LambdaLanczos<T>::SetMul(
    std::function<void(const std::vector<T> &, std::vector<T> &)> mv_mul) {
  this->mv_mul = mv_mul;
}
template <typename T>
inline void LambdaLanczos<T>::SetInitVec(
    std::function<void(std::vector<T> &)> init_vector) {
...
template <typename T> inline void LambdaLanczos<T>::SetEvalOffset(T offset) {
  this->eigenvalue_offset = offset;
}
template <typename T> inline void LambdaLanczos<T>::SetEpsilon(T epsilon) {
  this->eps = epsilon;
}
template <typename T>
...
template <typename T>
inline void VectorRandomInitializer<std::complex<T>>::init(
...
template <typename Scalar, typename Vector, typename ConstMatrix>
void PEigenDense<Scalar, Vector, ConstMatrix>::SetSize(int matrix_size) {
  n = matrix_size;
  evec.resize(n);
}
template <typename Scalar, typename Vector, typename ConstMatrix>
PEigenDense<Scalar, Vector, ConstMatrix>::PEigenDense(int matrix_size)
    : evec(matrix_size) {
  SetSize(matrix_size);
}
template <typename Scalar, typename Vector, typename ConstMatrix>
real_t<Scalar> PEigenDense<Scalar, Vector, ConstMatrix>::PrincipalEigen(
    ConstMatrix matrix, Vector eigenvector, bool find_max) {
