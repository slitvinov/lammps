  ...
  if (triclinic == 0) {
    if (xperiodic) {
      if (fabs(dx) > (MAXIMGCOUNT * xprd))
        error->one(FLERR,
                   "Atoms have moved too far apart ({}) for minimum image\n",
                   dx);
...
int Domain::ownatom(int, double *x, imageint *image, int shrinkexceed) {
  ...
  if (triclinic) {
    x2lamda(x, lamda);
    if (xperiodic && (lamda[0] < 0.0 || lamda[0] >= 1.0))
      lamda[0] = 0.0;
    if (yperiodic && (lamda[1] < 0.0 || lamda[1] >= 1.0))
      lamda[1] = 0.0;
    if (zperiodic && (lamda[2] < 0.0 || lamda[2] >= 1.0))
      lamda[2] = 0.0;
    coord = lamda;
  } else
    coord = x;
  ...
  if (coord[0] >= slo[0] && coord[0] < shi[0] && coord[1] >= slo[1] &&
      coord[1] < shi[1] && coord[2] >= slo[2] && coord[2] < shi[2])
    return 1;
  if (shrinkexceed) {
    int outside = 0;
    if (coord[0] < blo[0] && boundary[0][0] > 1)
      outside = 1;
    if (coord[0] >= bhi[0] && boundary[0][1] > 1)
      outside = 1;
    if (coord[1] < blo[1] && boundary[1][0] > 1)
      outside = 1;
    if (coord[1] >= bhi[1] && boundary[1][1] > 1)
      outside = 1;
    if (coord[2] < blo[2] && boundary[2][0] > 1)
      outside = 1;
    if (coord[2] >= bhi[2] && boundary[2][1] > 1)
      outside = 1;
    if (!outside)
      return 0;
    double newcoord[3];
    if (coord[0] < blo[0] && boundary[0][0] > 1)
      newcoord[0] = blo[0];
    else if (coord[0] >= bhi[0] && boundary[0][1] > 1)
      newcoord[0] = bhi[0];
    else
      newcoord[0] = coord[0];
    if (coord[1] < blo[1] && boundary[1][0] > 1)
      newcoord[1] = blo[1];
    else if (coord[1] >= bhi[1] && boundary[1][1] > 1)
      newcoord[1] = bhi[1];
    else
      newcoord[1] = coord[1];
    if (coord[2] < blo[2] && boundary[2][0] > 1)
      newcoord[2] = blo[2];
    else if (coord[2] >= bhi[2] && boundary[2][1] > 1)
      newcoord[2] = bhi[2];
    else
      newcoord[2] = coord[2];
    if (newcoord[0] >= slo[0] && newcoord[0] <= shi[0] &&
        newcoord[1] >= slo[1] && newcoord[1] <= shi[1] &&
        newcoord[2] >= slo[2] && newcoord[2] <= shi[2])
      return 1;
  }
  return 0;
}
...
void Domain::set_boundary(int narg, char **arg, int flag) {
  if (narg != 3)
    error->all(FLERR,
               "Illegal boundary command: expected 3 arguments but found {}",
               narg);
  char c;
  for (int idim = 0; idim < 3; idim++)
    for (int iside = 0; iside < 2; iside++) {
      if (iside == 0)
        c = arg[idim][0];
      else if (iside == 1 && strlen(arg[idim]) == 1)
        c = arg[idim][0];
      else
        c = arg[idim][1];
      if (c == 'p')
        boundary[idim][iside] = 0;
      else if (c == 'f')
        boundary[idim][iside] = 1;
      else if (c == 's')
        boundary[idim][iside] = 2;
      else if (c == 'm')
        boundary[idim][iside] = 3;
      else {
        if (flag == 0)
          error->all(FLERR, "Unknown boundary keyword: {}", c);
        if (flag == 1)
          error->all(FLERR, "Unknown change_box keyword: {}", c);
      }
    }
  for (int idim = 0; idim < 3; idim++)
  ...
  if (boundary[2][0] == 0)
    ...
    zperiodic = 0;
  int pflag = 0;
  if ((periodicity[0] && !xperiodic) || (periodicity[1] && !yperiodic) ||
      (periodicity[2] && !zperiodic))
    pflag = 1;
  periodicity[0] = xperiodic;
  periodicity[1] = yperiodic;
  ...
  if (xperiodic == 0 || yperiodic == 0 || zperiodic == 0) {
    nonperiodic = 1;
    if (boundary[0][0] >= 2 || boundary[0][1] >= 2 || boundary[1][0] >= 2 ||
        boundary[1][1] >= 2 || boundary[2][0] >= 2 || boundary[2][1] >= 2)
      nonperiodic = 2;
  }
  if (pflag) {
    pflag = 0;
    ...
    if ((flag_all > 0) && (comm->me == 0))
      error->warning(FLERR,
                     "Resetting image flags for non-periodic dimensions");
  }
}
...
void Domain::x2lamda(double *x, double *lamda, double *my_boxlo,
                     double *my_h_inv) {
  double delta[3];
  ...
  lamda[0] =
      my_h_inv[0] * delta[0] + my_h_inv[5] * delta[1] + my_h_inv[4] * delta[2];
...
void Domain::bbox(double *lo, double *hi, double *bboxlo, double *bboxhi) {
  ...
  x[2] = lo[2];
  lamda2x(x, x);
  bboxlo[0] = MIN(bboxlo[0], x[0]);
  bboxhi[0] = MAX(bboxhi[0], x[0]);
  bboxlo[1] = MIN(bboxlo[1], x[1]);
  bboxhi[1] = MAX(bboxhi[1], x[1]);
  bboxlo[2] = MIN(bboxlo[2], x[2]);
  bboxhi[2] = MAX(bboxhi[2], x[2]);
  x[0] = hi[0];
  x[1] = lo[1];
  x[2] = lo[2];
  lamda2x(x, x);
